...
import { 
    Search, Brain, Grid, Loader2, 
    BookOpen, Quote, User, Clipboard, CheckCircle 
} from 'lucide-react';
        ...
        <div className="p-6 md:p-8 bg-white rounded-xl shadow-md border-l-4 border-indigo-500 mb-6 transition-all duration-300 hover:shadow-lg relative">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between pt-4 border-t border-gray-100">
                ...
                <div className="flex flex-wrap gap-2 mt-2 md:mt-0">
                    {tags?.map((tag, index) => (
                        <span key={index} className="px-3 py-1 text-xs font-medium text-indigo-600 bg-indigo-50 rounded-full">
                            {tag}
                        </span>
                    ))}
                </div>
        ...
        <div className="min-h-screen bg-gray-100 rounded-lg font-sans p-4 sm:p-8">
            ...
            <header className="max-w-4xl mx-auto text-center py-10">
                ...
                <p className="text-lg text-gray-500 max-w-2xl mx-auto">
                    Search by author, theme, or book to discover verified quotes using Quote Finder, AI powered Book Quote Search Tool.
                </p>
            </header>
            ...
            <main className="max-w-6xl mx-auto">
                ...
                <section className="bg-white p-6 sm:p-8 rounded-xl shadow-2xl border border-gray-100 mb-12">
                    <h2 className="text-xl font-semibold text-gray-800 mb-6">Search Type</h2>
                    ...
                    <div className="flex space-x-2 mb-6 p-1 bg-gray-50 rounded-lg max-w-full overflow-x-auto">
                        {searchTypes.map((type) => (
                            <button
                                key={type}
                                onClick={() => { setActiveTab(type); setQuery(''); setResults(null); setError(null); }}
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
                        ...
                        <div className="relative">
                            ...
                            <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                                <svg className="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
                            </div>
                        </div>
                ...
                <section>
                    ...
                    {isPending && (
                        <div className="flex flex-col items-center justify-center p-12 bg-white rounded-xl shadow-lg">
                            ...
                            <p className="text-lg text-gray-600">Searching for verified quotes on '{query}'...</p>
                        </div>
                    )}
                    ...
                    {results === null && !isPending && !error && (
                        <div className="text-center p-12 bg-white rounded-xl shadow-lg border border-gray-100">
                            ...
                            <h3 className="text-xl font-semibold text-gray-700">Start Your Quote Search</h3>
                            <p className="text-gray-500">Enter a query above to find inspiring and accurate quotes powered by AI.</p>
                        </div>
                    )}
                    ...
                    {results && results.length === 0 && !isPending && (
                        <div className="text-center p-12 bg-yellow-50 rounded-xl shadow-lg border border-yellow-300">
                            ...
                            <h3 className="text-xl font-semibold text-gray-700">No Quotes Found</h3>
                            <p className="text-gray-500">The AI could not find any verifiable quotes matching your query. Please try different keywords.</p>
                        </div>
                    )}
                </section>
            </main>
