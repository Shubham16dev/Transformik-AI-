...
interface FeaturedToolsProps {
  limit?: number; // default if not passed
}
...
export function FeaturedTools({ limit = 5 }: FeaturedToolsProps) {
  ...
  useEffect(() => {
    const fetchFeaturedTools = async () => {
      setLoading(true);
      try {
        // Fetch a larger pool (e.g., 30)
        const { data, error } = await supabase
          .from("tools_summary")
          .select("id, tool_name, slug, logo, one_line_description, pricing_model, url, category")
          .limit(30);
  ...
  if (loading) return <p className="text-gray-500">Loading featured tools...</p>;

  if (!tools.length) return <p className="text-gray-500">No featured tools available.</p>;
